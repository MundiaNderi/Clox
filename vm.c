...
void push(Value value) {
    *vm.stackTop = value; // dereference the stack top pointer to store the value at the current top
                          // of the stack
...
#define READ_BYTE()                                                                                \
    (*vm.ip++) // reads the byte currently pointed at by ip and then advances the instruction
               // pointer
#define READ_CONSTANT()                                                                            \
    (vm.chunk->constants.values[READ_BYTE()]) // reads the next byte from the bytecode, treats the
                                              // resulting number as an index and looks up the
...
InterpretResult interpret(const char *source) {
    ...
    Chunk chunk;
    // Create a new empty chunk and pass it to the compiler
    initChunk(&chunk);
