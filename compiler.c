...
static void errorAt(Token *token, const char *message) {
    if (parser.panicMode)
        return;              // don't report errors if already in panic mode
    parser.panicMode = true; // enter panic mode to avoid cascading errors
...
// tell the user of the error token
...
// Reads the next token and validates that the token is of the expected type
...
// the first phase of compilation is scanning. Should return false if an error occured
bool compile(const char *source, Chunk *chunk) {
    ...
    expression(); // parses a single expression
